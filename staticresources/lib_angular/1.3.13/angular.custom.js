...
function createXhr() {
	/*
		This fixes issues with the modified XHR object in SF for pages that use ActionFunction
	*/
	var DEMO_USER_AGENT = navigator.userAgent.toLowerCase(),
		isXhrNative;
	
	if ( (DEMO_USER_AGENT.indexOf( "msie" ) > -1 || DEMO_USER_AGENT.indexOf( "trident" ) > -1 ) && DEMO_USER_AGENT.indexOf( "opera" ) == -1 ) {
		
		isXhrNative = ( window.XMLHttpRequest.toString().indexOf( "native" ) > -1 );
		
		if ( "Sarissa" in window && !!window.Sarissa[ "originalXMLHttpRequest" ] && !isXhrNative ) {			
